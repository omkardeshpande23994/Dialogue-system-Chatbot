...
def _get_random_bucket(train_buckets_scale):
    ...
    rand = random.random()
    return min(
        [i for i in range(len(train_buckets_scale)) if train_buckets_scale[i] > rand]
    )


def _assert_lengths(
    encoder_size, decoder_size, encoder_inputs, decoder_inputs, decoder_masks
):
...
def run_step(
    sess, model, encoder_inputs, decoder_inputs, decoder_masks, bucket_id, forward_only
):
    ...
    if not forward_only:
        output_feed = [
            model.train_ops[bucket_id],  # update op that does SGD.
...
def _find_right_bucket(length):
    ...
    return min(
        [b for b in range(len(config.BUCKETS)) if config.BUCKETS[b][0] >= length]
    )
...
def basic_Response(line):
    greetings = ("hi", "hey", "hello", "whatsup", "ssup")
    init_responses = ["hey", "hey man", "ssup", "hello ", "huh!!"]
    ...
    responses = [
        ...
        ["Top shelf scotch!!", "Gin and tonic!!", "Scotch on the rocks!!"],
        [
            "heh.. Please",
            "I smoke, drink and enjoy life, like a gentlemen!!",
            "I make people's life, Legen..DAAAARRRYYYY",
        ],
        ["Nice!!!!", "I had better!!!", "Have you gone madddd!!!!!"],
    ]

    if line.lower() in greetings:
        response = random.choice(init_responses)
        # print(response)
        return response
    elif line.lower() in questions:
        for i in range(len(questions)):
            if line.lower() == questions[i]:
