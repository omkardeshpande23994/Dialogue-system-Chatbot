...
class ChatBotModel:
    ...
    def _create_placeholders(self):
        ...
        self.encoder_inputs = [
            tf.compat.v1.placeholder(tf.int32, shape=[None], name="encoder{}".format(i))
            for i in range(config.BUCKETS[-1][0])
        ]
        self.decoder_inputs = [
            tf.compat.v1.placeholder(tf.int32, shape=[None], name="decoder{}".format(i))
            for i in range(config.BUCKETS[-1][1] + 1)
        ]
        self.decoder_masks = [
            tf.compat.v1.placeholder(tf.float32, shape=[None], name="mask{}".format(i))
            for i in range(config.BUCKETS[-1][1] + 1)
        ]
    ...
    def _inference(self):
        ...
        if config.NUM_SAMPLES > 0 and config.NUM_SAMPLES < config.DEC_VOCAB:
            ...
            b = tf.compat.v1.get_variable("proj_b", [config.DEC_VOCAB])
    ...
    def _creat_optimizer(self):
        ...
        with tf.compat.v1.variable_scope("training") as scope:
            self.global_step = tf.Variable(
                0, dtype=tf.int32, trainable=False, name="global_step"
            )

            if not self.fw_only:
                self.optimizer = tf.compat.v1.train.GradientDescentOptimizer(config.LR)
                ...
                for bucket in range(len(config.BUCKETS)):
                    ...
                    print(
                        "Creating opt for bucket {} took {} seconds".format(
